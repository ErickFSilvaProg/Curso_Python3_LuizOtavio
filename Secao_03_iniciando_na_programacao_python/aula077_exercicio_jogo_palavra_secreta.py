"""
Faça um jogo para o usuário adivinhar qual a palavra secreta.

    1. Você vai propor uma palavra secreta qualquer e vai dar a possibilidade para o usuário digitar apenas uma letra.
    2. Quando o usuário digitar uma letra, você vai conferir se a letra digitada está na palavra secreta.
    3. Se a letra digitada estiver na palavra secreta; exiba a letra;
    4. Se a letra digitada não estiver na palavra secreta; exiba *.

Faça a contagem de tentativas do seu usuário.


Comando Python para limpar o terminal:
    
    import os
        os.system('cls')

"""

palavra_secreta = 'perfume'
letras_acertadas = ''
numero_tentativas = 0

while True:
    palavra_formada = ''
    letra_digitada = input('Digite uma letra: ')
    
    numero_tentativas += 1
    
    if len(letra_digitada) > 1:
        print('Digite apenas uma letra.')
        print()
        continue
    
    if letra_digitada in palavra_secreta:
        letras_acertadas += letra_digitada
    
    for letra_secreta in palavra_secreta:
        if letra_secreta in letras_acertadas:
            palavra_formada += letra_secreta
        else:
            palavra_formada += '*'
    
    print('Paralvra formada:', palavra_formada)
    print()
    
    if palavra_formada == palavra_secreta:
        print('Você ganhou, parabéns!!!')
        print('A palavra era', palavra_secreta)
        print('Tentativas:', numero_tentativas)
        print()